...
export function useCanvasHandlers({
  ...
  const updateQrcode = useCallback(
    async (
      ...
      // --- PERBAIKAN DI SINI ---
      // Akses properti langsung dari objek 'options'
      const data =
  ...
  const handlePropertyChange = (prop: string, value: any) => {
    ...
    if (prop === "fill" && (activeObject as any).isQrcode) {
      void updateQrcode(activeObject, { newColor: value });
      return;
    }
  ...
  const handleQrcodeDataChange = (data: string) => {
    ...
    (activeObject as any).qrcodeData = data;
  ...
  const handleAddQrcode = useCallback(async () => {
    ...
    try {
      ...
      (img as any).qrcodeData = qrcodeValue;
      (img as any).qrcodeFill = "#000000";
